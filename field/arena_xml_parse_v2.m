function [my_modes,my_range_gates,socket_payload_size] = arena_xml_parse_v2(filename)
...
for k = 0:all_digRx.getLength-1
  ...
  first = this_element.getFirstChild.getData;
  temp = str2double(strsplit(char(first), ':'));
  this_modes=[temp(1) floor(temp(1))+1:floor(temp(end))];	%gets range of the modes
...
for k = 0:all_integrator.getLength-1
  ...
  first = this_modes_element.getFirstChild.getData;
  temp = str2double(strsplit(char(first), ':'));
  this_mode=[temp(1) floor(temp(1))+1:floor(temp(end))];		%gets range of the modes
